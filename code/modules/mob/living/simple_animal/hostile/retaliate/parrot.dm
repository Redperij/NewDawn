				...
				if("ears")
					if(ears)
						...
						for(var/ch in headset_to_add.channels)
							switch(ch)
								if("Инженерный")
									available_channels.Add(":и")
								if("Командный")
									available_channels.Add(":к")
								if("Сб")
									available_channels.Add(":б")
								if("Научный")
									available_channels.Add(":у")
								if("Медицинский")
									available_channels.Add(":м")
								if("Снабжение")
									available_channels.Add(":т")
								if("Сервис")
									available_channels.Add(":с")
			return TOPIC_HANDLED

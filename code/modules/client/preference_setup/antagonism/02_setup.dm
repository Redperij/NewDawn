...
/datum/category_item/player_setup_item/antagonism/basic/content(var/mob/user)
	. +="<b>Настройка Антага:</b><br>"
	. +="Приоритет Источника Аплинка: <a href='?src=\ref[src];add_source=1'>Добавить</a><br>"
	for(var/entry in pref.uplink_sources)
		var/decl/uplink_source/US = entry
		. +="[US.name] <a href='?src=\ref[src];move_source_up=\ref[US]'>Поднять</a> <a href='?src=\ref[src];move_source_down=\ref[US]'>Понизить</a> <a href='?src=\ref[src];remove_source=\ref[US]'>Убрать</a><br>"
		if(US.desc)
			. += "<font size=1>[US.desc]</font><br>"
	if(!pref.uplink_sources.len)
		. += "<span class='warning'>Если вы не добавите источник аплинка, то вы его не получите!</span>"
	. +="<br>"
	. +="Полезная Информация:<br>"
	if(jobban_isbanned(user, "Records"))
		. += "<b>Вам запрещено использовать записи.</b><br>"
	else
...
/datum/category_item/player_setup_item/antagonism/basic/OnTopic(var/href,var/list/href_list, var/mob/user)
	if(href_list["add_source"])
		var/source_selection = input(user, "Выберите Источник Аплинка", CHARACTER_PREFERENCE_INPUT_TITLE) as null|anything in (list_values(uplink_sources_by_name) - pref.uplink_sources)
	...
	if(href_list["exploitable_record"])
		var/exploitmsg = sanitize(input(user,"Напишите полезную информацию о себе ( которую можно будет использовать ).","Полезная Информация", html_decode(pref.exploit_record)) as message|null, MAX_PAPER_MESSAGE_LEN, extra = 0)

...
/datum/preferences
	...
	var/h_style = "Лысый"				//Hair type
	var/r_hair = 0						//Hair color
	var/g_hair = 0						//Hair color
	var/b_hair = 0						//Hair color
	var/f_style = "Бритый"				//Face hair type
...
/datum/category_item/player_setup_item/physical/body/content(var/mob/user)
	...
	if(has_flag(mob_species, HAS_A_SKIN_TONE))
		. += "Оттенок Кожи: <a href='?src=\ref[src];skin_tone=1'>[-pref.s_tone + 35]/[mob_species.max_skin_tone()]</a><br>"

	. += "Нужно Очки: <a href='?src=\ref[src];disabilities=[NEARSIGHTED]'><b>[pref.disabilities & NEARSIGHTED ? "Да" : "Нет"]</b></a><br>"
	. += "Конечности: <a href='?src=\ref[src];limbs=1'>Настроить</a> <a href='?src=\ref[src];reset_limbs=1'>Сбросить</a><br>"
	. += "Внутренние Органы: <a href='?src=\ref[src];organs=1'>Настроить</a><br>"

	//display limbs below
	var/ind = 0
	for(var/name in pref.organ_data) //tbt
		...
		switch(name)
			...
			if(BP_CHEST)
				organ_name = "верхнее тело"
			if(BP_GROIN)
				organ_name = "нижнее тело"
			if(BP_HEAD)
				organ_name = "голова"
		...
		if(status == "cyborg")
			...
			if(pref.rlimb_data[name] && all_robolimbs[pref.rlimb_data[name]])
				...
				R = basic_robolimb
			. += "\t[R.company] [organ_name] протез"
		else if(status == "amputated")
			++ind
			if(ind > 1)
				. += ", "
			. += "\tАмпутированная [organ_name]"
		else if(status == "mechanical")
			++ind
			if(ind > 1)
				. += ", "
			if(organ_name == BP_BRAIN)
				. += "\tПозитронная [organ_name]"
			else
				. += "\tСинтетическая [organ_name]"
			...
			switch(organ_name)
				...
				if("voicebox") //on adding voiceboxes for speaking skrell/similar replacements
					. += "\tХирургически-изменённый орган: [organ_name]"
				if(BP_EYES)
					. += "\tСечаточный Интерфейс [organ_name]"
				if(BP_BRAIN)
					. += "\t[organ_name] с машинным интерфесом"
				else
					. += "\tИскусственно поддерживаемый орган: [organ_name]"
	if(!ind)
		. += "\[...\]<br><br>"
	else
		. += "<br><br>"
	...
	. += "<br><a href='?src=\ref[src];toggle_preview_value=[EQUIP_PREVIEW_LOADOUT]'>[pref.equip_preview_mob & EQUIP_PREVIEW_LOADOUT ? "Скрыть снаряжение" : "Показать снаряжение"]</a>"
	. += "<br><a href='?src=\ref[src];toggle_preview_value=[EQUIP_PREVIEW_JOB]'>[pref.equip_preview_mob & EQUIP_PREVIEW_JOB ? "Скрыть рабочее снаряжение" : "Показать рабочее снаряжение"]</a>"
	. += "</td></tr></table>"

	. += "<b>Волосы</b><br>"
	if(has_flag(mob_species, HAS_HAIR_COLOR))
		. += "<a href='?src=\ref[src];hair_color=1'>Изменить Цвет</a> <font face='fixedsys' size='3' color='#[num2hex(pref.r_hair, 2)][num2hex(pref.g_hair, 2)][num2hex(pref.b_hair, 2)]'><table style='display:inline;' bgcolor='#[num2hex(pref.r_hair, 2)][num2hex(pref.g_hair, 2)][num2hex(pref.b_hair)]'><tr><td>__</td></tr></table></font> "
	. += " Стиль: <a href='?src=\ref[src];hair_style=1'>[pref.h_style]</a><br>"

	. += "<br><b>Лицо</b><br>"
	if(has_flag(mob_species, HAS_HAIR_COLOR))
		. += "<a href='?src=\ref[src];facial_color=1'>Изменить Цвет</a> <font face='fixedsys' size='3' color='#[num2hex(pref.r_facial, 2)][num2hex(pref.g_facial, 2)][num2hex(pref.b_facial, 2)]'><table  style='display:inline;' bgcolor='#[num2hex(pref.r_facial, 2)][num2hex(pref.g_facial, 2)][num2hex(pref.b_facial)]'><tr><td>__</td></tr></table></font> "
	. += " Стиль: <a href='?src=\ref[src];facial_style=1'>[pref.f_style]</a><br>"

	if(has_flag(mob_species, HAS_EYE_COLOR))
		. += "<br><b>Глаза</b><br>"
		. += "<a href='?src=\ref[src];eye_color=1'>Изменить Цвет</a> <font face='fixedsys' size='3' color='#[num2hex(pref.r_eyes, 2)][num2hex(pref.g_eyes, 2)][num2hex(pref.b_eyes, 2)]'><table  style='display:inline;' bgcolor='#[num2hex(pref.r_eyes, 2)][num2hex(pref.g_eyes, 2)][num2hex(pref.b_eyes)]'><tr><td>__</td></tr></table></font><br>"

	if(has_flag(mob_species, HAS_SKIN_COLOR))
		. += "<br><b>Цвет Тела</b><br>"
		. += "<a href='?src=\ref[src];skin_color=1'>Изменить Цвет</a> <font face='fixedsys' size='3' color='#[num2hex(pref.r_skin, 2)][num2hex(pref.g_skin, 2)][num2hex(pref.b_skin, 2)]'><table style='display:inline;' bgcolor='#[num2hex(pref.r_skin, 2)][num2hex(pref.g_skin, 2)][num2hex(pref.b_skin)]'><tr><td>__</td></tr></table></font><br>"

	. += "<br><a href='?src=\ref[src];marking_style=1'>Ответки на Теле +</a><br>"
...
/datum/category_item/player_setup_item/physical/body/OnTopic(var/href,var/list/href_list, var/mob/user)
	...
	if(href_list["toggle_species_verbose"])
		...
		var/choice = input("На какой вид вы желаете посмотреть?") as null|anything in playable_species
		if(choice)
		...
		var/choice = input("Выберите доступный вид.") as null|anything in species_to_pick
		...
		var/new_hair = input(user, "Выберите цвет волос:", CHARACTER_PREFERENCE_INPUT_TITLE, rgb(pref.r_hair, pref.g_hair, pref.b_hair)) as color|null
		...
		var/new_h_style = input(user, "Выберите причёску:", CHARACTER_PREFERENCE_INPUT_TITLE, pref.h_style)  as null|anything in valid_hairstyles
		...
		if(!has_flag(mob_species, HAS_HAIR_COLOR))
			return TOPIC_NOACTION
		var/new_facial = input(user, "Выберите цвет волос на лице:", CHARACTER_PREFERENCE_INPUT_TITLE, rgb(pref.r_facial, pref.g_facial, pref.b_facial)) as color|null
		...
		if(!has_flag(mob_species, HAS_BASE_SKIN_COLOURS))
			return TOPIC_NOACTION
		var/new_s_base = input(user, "Выберите цвет персонажа:", CHARACTER_PREFERENCE_INPUT_TITLE) as null|anything in mob_species.base_skin_colours
		...
		if(!has_flag(mob_species, HAS_A_SKIN_TONE))
			return TOPIC_NOACTION
		var/new_s_tone = input(user, "Выберите оттенок кожи. Чем меньше число - тем светлее, больше - темнее. Промежуток: от 1 до [mob_species.max_skin_tone()]", CHARACTER_PREFERENCE_INPUT_TITLE, (-pref.s_tone) + 35) as num|null
		mob_species = all_species[pref.species]
		...
		var/new_f_style = input(user, "Выберите стиль волос на лице:", CHARACTER_PREFERENCE_INPUT_TITLE, pref.f_style)  as null|anything in valid_facialhairstyles
		...
		var/new_marking = input(user, "Выберите отметку на теле:", CHARACTER_PREFERENCE_INPUT_TITLE)  as null|anything in usable_markings
		...
		var/mark_color = input(user, "[M]. Выберите цвет: ", CHARACTER_PREFERENCE_INPUT_TITLE, pref.body_markings[M]) as color|null
		...
		var/list/limb_selection_list = list("Левая Нога","Правая Нога","Левая Рука","Правая Рука","Левая Стопа","Правая Стопа","Левая Кисть","Правая Кисть","Всё Тело") //tbt
		...
		var/list/choice_options = list("Нормальная","Ампутированная","Протез")
		...
		if(pref.organ_data[BP_CHEST] == "cyborg")
			choice_options = list("Ампутированнная", "Протез")
		...
		var/new_state = input(user, "В каком состоянии вы желаете видеть конечность?") as null|anything in choice_options
		...
		switch(new_state)
			...
			if("Протез")
				...
				if(!usable_manufacturers.len)
					return
				var/choice = input(user, "Кто является производителем протеза?") as null|anything in usable_manufacturers
				if(!choice)
					return
		...
		var/organ_name = input(user, "Какой внутренний орган вы хотите изменить?") as null|anything in list("Сердце", "Глаза", "Лёгкие", "Печень", "Почки", "Желудок") //tbt
		...
		var/list/organ_choices = list("Нормальный","Поддерживаемый","Синтетический")

		if(mob_species && mob_species.spawn_flags & SPECIES_NO_ROBOTIC_INTERNAL_ORGANS)
			organ_choices -= "Поддерживаемый"
		...
		var/new_state = input(user, "В каком состоянии вы желаете видеть орган?") as null|anything in organ_choices
		if(!new_state) return
...
/datum/category_item/player_setup_item/proc/ResetHair()
	...
	if(valid_hairstyles.len)
		...
		pref.h_style = GLOB.hair_styles_list["Лысый"]
...
/datum/category_item/player_setup_item/proc/ResetFacialHair()
	...
	if(valid_facialhairstyles.len)
		...
		//this shouldn't happen
		pref.f_style = GLOB.facial_hair_styles_list["Бритый"]

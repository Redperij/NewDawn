...
/datum/category_item/player_setup_item/physical/basic/OnTopic(var/href,var/list/href_list, var/mob/user)
	...
	if(href_list["rename"])
		var/raw_name = input(user, "Выберите Имя Своего Персонажа:", "Имя Персонажа")  as text|null
		if (!isnull(raw_name) && CanUseTopic(user))
			...
			if(new_name)
				...
				to_chat(user, "<span class='warning'>Неприемлемое имя. Ваше имя должно состоять из 2 до [MAX_NAME_LEN] символов. Допустимые символы: A-Z, a-z, -, ', ., А-Я, а-я</span>")
		...
		for(var/spawntype in spawntypes())
			spawnkeys += spawntype
		var/choice = input(user, "Где вы предпочли бы появляться?") as null|anything in spawnkeys

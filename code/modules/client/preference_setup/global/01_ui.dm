...
/datum/category_item/player_setup_item/player_global/ui/content(var/mob/user)
	. += "<b>Настройки Интерфейса (UI)</b><br>"
	. += "<b>Стиль UI:</b> <a href='?src=\ref[src];select_style=1'><b>[pref.UI_style]</b></a><br>"
	. += "<b>Пользовательский UI</b> (рекомендуется для Белого UI):<br>"
	. += "-Цвет: <a href='?src=\ref[src];select_color=1'><b>[pref.UI_style_color]</b></a> <table style='display:inline;' bgcolor='[pref.UI_style_color]'><tr><td>__</td></tr></table> <a href='?src=\ref[src];reset=ui'>сбросить</a><br>"
	...
	if(can_select_ooc_color(user))
		...
		if(pref.ooccolor == initial(pref.ooccolor))
			. += "<a href='?src=\ref[src];select_ooc_color=1'><b>Используется Стандартный</b></a><br>"
		else
			. += "<a href='?src=\ref[src];select_ooc_color=1'><b>[pref.ooccolor]</b></a> <table style='display:inline;' bgcolor='[pref.ooccolor]'><tr><td>__</td></tr></table> <a href='?src=\ref[src];reset=ooc'>сбросить</a><br>"
	. += "<b>Клиентский FPS:</b> <a href='?src=\ref[src];select_fps=1'><b>[pref.clientfps]</b></a><br>"

/datum/category_item/player_setup_item/player_global/ui/OnTopic(var/href,var/list/href_list, var/mob/user)
	if(href_list["select_style"])
		var/UI_style_new = input(user, "Выберите стиль UI.", CHARACTER_PREFERENCE_INPUT_TITLE, pref.UI_style) as null|anything in all_ui_styles
		if(!UI_style_new || !CanUseTopic(user)) return TOPIC_NOACTION
		pref.UI_style = UI_style_new
		return TOPIC_REFRESH

	else if(href_list["select_color"])
		var/UI_style_color_new = input(user, "Выберите цвет UI, рекомендуются тёмные цвета!", "Глобальная Настройка", pref.UI_style_color) as color|null
		...
		var/UI_style_alpha_new = input(user, "Выберите уровень альфы (прозрачности) UI, Между 50 и 255.", "Глобальная Настройка", pref.UI_style_alpha) as num|null
		...
		var/new_ooccolor = input(user, "Выберите цвет OOC:", "Глобальная Настройка") as color|null
		...
		var/new_fps = input(user, "Выберите клиентский фпс.[version_message]\n(0 = синхронизируется с сервером (это:[world.fps]))", "Глобальная Настройка") as num|null
